...
同一化される範囲は指定した形式によって異なります。NFC と NFD は正準等価な文字列を揃え、NFKC と NFKD は互換等価な文字列も揃えます。どの形式であっても異体字セレクタ（葛 + U+E0101）は2コードポイントのままですし、ユーザーが1文字と認識する単位で数えたい場合は書記素クラスタの分割が必要で、これは正規化とは別の処理です。
