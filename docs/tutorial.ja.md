...
`minLength` / `maxLength` / `fixedLength` はコードポイントで数えますが、同じ「が」でも合成済み（U+304C）と分解済み（U+304B U+3099）ではコードポイント数が違います。macOS のファイル名は分解済みですし、IME やクリップボード経由でも分解済みのテキストが届くことがあります。`normalize()` を前に挟むと、どちらの形で届いても同じ数え方になります。

```java
String nfd = "か\u3099";   // 「が」の分解形（U+304B U+3099）
...
デフォルトは NFC で、`normalize(Normalizer.Form.NFKC)` のように形式を指定することもできます。NFKC は半角ｱと全角アの区別まで消すので、検索キーには向きますが、名前を保存する用途には向きません。

`trim()` や `toLowerCase()` と同じ変換なので、書いた順に効きます。`maxLength(20).normalize()` と書けば、届いたままの形で長さを検査してから正規化します。

正規化が揃えるのは正準等価な文字列だけです。異体字セレクタ（葛 + U+E0101）は正規化しても2コードポイントのままですし、ユーザーが1文字と認識する単位で数えたい場合は書記素クラスタの分割が必要で、これは正規化とは別の処理です。
