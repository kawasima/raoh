 ...
 */
public record Combiner7<I, A, B, C, D, E, F, G>(CombinePart<I, A> da, CombinePart<I, B> db, CombinePart<I, C> dc, CombinePart<I, D> dd, CombinePart<I, E> de, CombinePart<I, F> df, CombinePart<I, G> dg) {
    ...
    public <T> Decoder<I, T> map(Function7<A, B, C, D, E, F, G, T> f) {
        return (in, path) -> {
            ...
            return Validated.accumulate(
                    new Validated<?>[]{ va, vb, vc, vd, ve, vf, vg },
                    args -> f.apply((A) args[0], (B) args[1], (C) args[2], (D) args[3], (E) args[4], (F) args[5], (G) args[6])
            ).toResult();
        };
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> Decoder<I, T> flatMap(Function7<A, B, C, D, E, F, G, Result<T>> f) {
        return (in, path) -> {
            var va = Validated.fromResult(da.decode(in, path));
            var vb = Validated.fromResult(db.decode(in, path));
            var vc = Validated.fromResult(dc.decode(in, path));
