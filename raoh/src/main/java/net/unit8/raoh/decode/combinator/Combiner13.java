 ...
 */
public record Combiner13<I, A, B, C, D, E, F, G, H, J, K, L, M, N>(CombinePart<I, A> da, CombinePart<I, B> db, CombinePart<I, C> dc, CombinePart<I, D> dd, CombinePart<I, E> de, CombinePart<I, F> df, CombinePart<I, G> dg, CombinePart<I, H> dh, CombinePart<I, J> dj, CombinePart<I, K> dk, CombinePart<I, L> dl, CombinePart<I, M> dm, CombinePart<I, N> dn) {
    ...
    public <T> Decoder<I, T> map(Function13<A, B, C, D, E, F, G, H, J, K, L, M, N, T> f) {
        return (in, path) -> {
            ...
            return Validated.accumulate(
                    new Validated<?>[]{ va, vb, vc, vd, ve, vf, vg, vh, vj, vk, vl, vm, vn },
                    args -> f.apply((A) args[0], (B) args[1], (C) args[2], (D) args[3], (E) args[4], (F) args[5], (G) args[6], (H) args[7], (J) args[8], (K) args[9], (L) args[10], (M) args[11], (N) args[12])
            ).toResult();
        };
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> Decoder<I, T> flatMap(Function13<A, B, C, D, E, F, G, H, J, K, L, M, N, Result<T>> f) {
        return (in, path) -> {
            var va = Validated.fromResult(da.decode(in, path));
            var vb = Validated.fromResult(db.decode(in, path));
            var vc = Validated.fromResult(dc.decode(in, path));
