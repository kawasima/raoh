 ...
 *
 * <p><strong>Known limitation:</strong> The weaver identifies the super/this constructor
 * call as the first {@code invokespecial <init>} targeting the superclass or this class.
 * If constructor arguments themselves instantiate the superclass or this class
 * (e.g., {@code super(new Super())}), the detection may be incorrect. Such patterns
 * are extremely rare in domain classes.
 */
public final class GuardWeaver {
     ...
     *
     * <p>Detection is based on the first {@code invokespecial <init>} targeting the same class.
     *
    ...
    private static boolean isDelegatingConstructor(CodeModel code, String thisClassName) {
        for (var element : code) {
            if (element instanceof InvokeInstruction inv
                    && inv.opcode() == Opcode.INVOKESPECIAL
                    && inv.name().equalsString("<init>")) {
                // The first invokespecial <init> in a constructor is always
                // the chain call (super or this). If it targets this class, it's this(...).
                return inv.owner().asInternalName().equals(thisClassName);
            }
        }
        return false;
    }
