...
public class UserRepository {
    ...
    public UserId insert(String name, EmailAddress email) {
        // Encode the domain object into a flat column map — the write-side mirror of decoding a
        // JDBC row with MapMembershipDecoders.USER_ROW. The id is generated by the database, so we
        // pass a placeholder (never persisted): the INSERT only writes the name/email columns.
        User draft = new User(new UserId(0L), name, email);
        Map<String, Object> row = MapMembershipEncoders.USER_ROW.encode(draft);
